  ...
  */
  void handle_driving(uint8_t joy_x, uint8_t joy_y, uint8_t joy_r, uint8_t speed, bool smoothly) {
    ...
    if (joy_y >= 150) {
      ...
      left_track_forward_direction = true;
      right_track_forward_direction = true;
      left_track_power = value;
      right_track_power = value;
      block_rotation = true;
    } else if (joy_y <= 120) {
      uint8_t value = map(joy_y, 120, 0, 0, speed);
      left_track_forward_direction = false;
      right_track_forward_direction = false;
      left_track_power = value;
      right_track_power = value;
      block_rotation = true;
    }
    ...
    if (smoothly) {
      set_smoothly_left_track(left_track_power, left_track_forward_direction);
      set_smoothly_right_track(right_track_power, right_track_forward_direction);
    } else {
      set_left_track(left_track_power, left_track_forward_direction);
      set_right_track(right_track_power, right_track_forward_direction);
    }
  }

  void enable_motors(bool is_enabled) {
  ...
  bool left_track_forward_direction = true;
  bool right_track_forward_direction = true;

private:

  uint8_t latest_left_track_power = 0;
  uint8_t latest_right_track_power = 0;
  bool latest_left_forward = true;
  bool latest_right_forward = true;
